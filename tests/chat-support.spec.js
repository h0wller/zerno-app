...
test('выбор доставки открывает доставочную Нику', async ({ page }) => {
  ...
  await expect(page.locator('.chatHint').first()).toBeVisible({ timeout: 5000 }); // подсказки не глотаются
});
...
test('вызов сотрудника требует подтверждения (два тапа)', async ({ page }) => {
  ...
  await call.click();
  await expect(page.locator('.chatHint', { hasText: 'Точно позвать' })).toBeVisible(); // первый тап не отправляет
});