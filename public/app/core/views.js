...
(function () {
  ...
  var SUPPORT_ENTRY =
    QS.get("tab") === "chat" || QS.get("support") === "choose";
  var chosenSupportCtx = SUPPORT_ENTRY
    ? sessionStorage.getItem("zt_support_ctx") || ""
    : "";
  ...
  window.__fvChatState = {
    ...
    setChatCtx: function (v) {
      chatCtx = v;
      try {
        localStorage.setItem("zt_chatctx", v);
      } catch (e) {}
    },
    getSupportPending: function () {
      return supportPending;
    },
    setSupportPending: function (v) {
      supportPending = v;
    },
    getChosenSupportCtx: function () {
      return chosenSupportCtx;
    },
    setChosenSupportCtx: function (v) {
      chosenSupportCtx = v;
      try {
        sessionStorage.setItem("zt_support_ctx", v);
      } catch (e) {}
    },
  };
  ...
  var rules = [
    ...
    '[data-brand="delivery"] .topbar .brand .mark{width:auto;height:40px;max-width:min(56vw,240px);border-radius:8px}',
    '@media(max-width:820px){[data-brand="delivery"] .topbar{flex-wrap:wrap}[data-brand="delivery"] .topbar .brand{order:0;flex:1 1 100%;width:100%;justify-content:center;margin:0 0 4px}[data-brand="delivery"] .topbar .venueWrap{order:1}[data-brand="delivery"] .topbar #clock{order:2;margin-left:auto}[data-brand="delivery"] .topbar #profileTopBtn{order:3}[data-brand="delivery"] .topbar #modeSeg{order:4;flex:1 1 100%}[data-brand="delivery"] .topbar .brand .mark{width:100%;max-width:100%;height:56px;max-height:56px;overflow:hidden;flex:0 0 auto}[data-brand="delivery"] .topbar .brand .mark img,[data-brand="delivery"] .topbar .brand .mark svg{width:100%;height:100%!important;max-height:none;object-fit:contain;border-radius:8px}}',
  ...
  if (sec && ab0 && ab0.parentNode !== sec)
    sec.insertBefore(ab0, sec.firstChild);
  ...
  (function () {
    ...
    if (ov && !document.getElementById("pendingBoxD")) {
      ...
      d.innerHTML =
        '<h3 style="margin:0 0 8px">🆕 Активация гостей</h3><div id="pendingBoxD"></div>';
      var lc = document.getElementById("ordersList");
      if (lc)
        ov.querySelector(".cashier").insertBefore(d, lc.closest(".cash-card"));
    }
    if (ov && !document.getElementById("chatsToggleD")) {
      ...
      b.onclick =
        typeof openStaffChat === "function" ? openStaffChat : function () {};
  ...
  function sv() {
    try {
      ...
      var bName =
        typeof brand !== "undefined" && brand === "delivery"
          ? "delivery"
          : "coffee";
    ...
    sv() владеет только классом марки и текстами #brandTitle/#brandSub. */
      ...
      if (op) {
        if (!op.hasAttribute("data-orig"))
          op.setAttribute("data-orig", op.innerHTML);
      ...
      if (bSub)
        bSub.textContent = isDel
          ? "доставка пиццы и роллов"
          : "кофейня на берегу моря";
      ...
      if (seg2 && !document.getElementById("venueToggle")) {
        ...
        vw.appendChild(vt);
        vw.appendChild(seg2); // #brandSeg = список дропдауна (тесты кликают его)
      ...
      if (vt2)
        vt2.innerHTML =
          (isDel ? "🍕 Пятница" : "🌊 Кофейня") +
          ' <span class="vt-arrow">▾</span>';
    } catch (e) {}

    var showGuest =
      typeof mode !== "undefined" && (mode === "guest" || mode === "admin");
    var bName2 =
      typeof brand !== "undefined" && brand === "delivery"
        ? "delivery"
        : "coffee";
    ...
    if (mv) {
      mv.hidden = !showCoffee;
      mv.style.display = showCoffee ? "" : "none";
    }
    if (dv) {
      dv.hidden = false;
      dv.style.display = showDeliv ? "block" : "none";
    }
    ...
    var isStaff = !!(
      typeof me !== "undefined" &&
      me &&
      (me.role === "admin" || me.role === "cashier" || me.role === "dispatch")
    );
    ...
    if (mb)
      mb.style.display = mode === "guest" && bName2 === "coffee" ? "" : "none";

    var bt = document.querySelector('.tabs button[data-tab="bonus"]');
    if (bt) bt.style.display = bName2 === "delivery" ? "none" : "";

    if (bName2 === "delivery" && typeof setTab === "function")
      setTab("profile");
    if (typeof me !== "undefined" && me && typeof renderProfile === "function")
      renderProfile();

    var staff =
      typeof mode !== "undefined" && (mode === "cashier" || mode === "orders");
    ...
    document
      .querySelectorAll("#scanBtn,#scanFab,#qrFab,#scanToggle,.fab-scan")
      .forEach(function (b) {
    ...
    if (typeof window.applyBrandChrome === "function")
  ...
  window.setMode = function (m) {
    ...
    var pt = document.getElementById("promoToggle");
    if (pt) pt.hidden = m !== "admin";
    var dt = document.getElementById("dashToggle");
    ...
    var bd = document.getElementById("adminBadge");
    ...
    if (m === "orders") {
      ...
      if (typeof ordersPoll === "undefined" || !ordersPoll) {
        window.ordersPoll = setInterval(function () {
          if (mode === "orders" && typeof renderOrders === "function")
            renderOrders(true);
        }, 8000);
      }
    }
    ...
    if (
      (m === "guest" || m === "admin") &&
      brand === "delivery" &&
      (typeof DMENU === "undefined" || !DMENU.length) &&
      typeof loadDelivery === "function"
    ) {
      loadDelivery();
    }
    if (typeof renderModes === "function") renderModes();
  ...
  if (brandSegEl) {
    brandSegEl.addEventListener("click", function (e) {
      ...
      try {
        localStorage.setItem("zt_brand", brand);
      } catch (e) {}
      document.documentElement.setAttribute("data-brand", brand);

      if (
        typeof mode !== "undefined" &&
        (mode === "cashier" || mode === "orders")
      ) {
      ...
      if (
        typeof supportPending !== "undefined" &&
        !supportPending &&
        typeof chatCtx !== "undefined" &&
        chatCtx !== brand
      ) {
        chatCtx = brand;
        try {
  ...
  (function () {
    function findPush() {
      ...
      for (var i = 0; i < all.length; i++) {
        ...
        if (
          t.indexOf("Включите пуши") > -1 &&
          t.length < 40 &&
          all[i].children.length <= 2
        )
          return all[i];
      }
      return null;
    ...
    function pushBtnEl() {
      ...
      for (var i = 0; i < all.length; i++) {
        if (/Включить уведомления/.test(all[i].textContent || ""))
          return all[i];
      }
      return null;
    }
