/* public/app/core/views.js — Ф3.22/Ф3.41: флаги и state-мост чата, CSS-инъекция (СЛОЙ 2),
DOM-переезды, виды/режимы (sv/setMode/brandSeg), дропдаун заведений, пуш-баббл.
Полная перезапись Ф3.41: убраны дубли селекторов, сиротские @media, небаланс скобок.
...
'[data-brand="coffee"] .topbar .brand .mark{width:calc(var(--topbar-h,72px) - 16px);height:calc(var(--topbar-h,72px) - 16px);border-radius:14px}',
...
'[data-brand="delivery"] .venueToggle{border:2px solid var(--fr-choc);bacskground:var(--fr-paper);color:var(--fr-choc);box-shadow:2px 2px 0 var(--fr-choc)}',
...
'@media(min-width:821px){.topbar{position:relative;display:flex;flex-wrap:wrap;align-items:center;gap:8px;height:auto;min-height:calc(var(--topbar-h,64px) + env(safe-area-inset-top,0px))}.topbar .venueWrap{order:1;flex:0 0 auto;margin-right:auto}.topbar .brand{position:absolute;left:50%;top:calc((var(--topbar-h,64px) + env(safe-area-inset-top,0px)) / 2);transform:translate(-50%,-50%);order:0;flex:none;max-width:60%;margin:0}.topbar #profileTopBtn{order:2;flex:0 0 auto;margin-left:0}.topbar #modeSeg{order:10;flex:1 1 100%}}',
'@media(min-width:821px){[data-brand="delivery"] .topbar .brand .mark{height:calc(var(--topbar-h,64px) - 8px);max-width:min(46vw,420px)}[data-brand="coffee"] .topbar .brand .mark{width:calc(var(--topbar-h,64px) - 10px);height:calc(var(--topbar-h,64px) - 10px)}}',
