...
/* ── Ф3.10: пост-обработка карточек доставки (стоп-лист, фото, карандаши, тумблеры, размеры) ── */
(function(){
var s=document.createElement('style');
s.textContent='#deliveryGrid .card .media img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center;display:block}';
...
if(!card.querySelector('.edBtn')){var eb=document.createElement('button');eb.type='button';eb.className='edBtn';eb.dataset.ed=p.id;eb.textContent='✏️';card.appendChild(eb);}
...
new MutationObserver(function(){patchCards();}).observe(document.getElementById('deliveryGrid')||document.body,{childList:true,subtree:true});
...
/* ЕДИНСТВЕННЫЙ обработчик стоп-листа */
...
/* стоп-позиция не добавляется; пицца требует размер */
