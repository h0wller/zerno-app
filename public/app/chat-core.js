  ...
  window.fetch = function(u, o){
    ...
    try {
      if (o && o.method === 'POST' && String(u).indexOf('/api/push/send') > -1) {
        pr.then(function(r){ return r.clone().json(); })
          .then(function(j){
            setTimeout(function(){
              toast('Доставлено: ' + j.delivered + ' · Ошибок: ' + j.failed +
...
window.setBotName = function(){
  ...
  var name = CS.getSupportPending() ? 'Ника · поддержка' :
             (CS.getChatCtx() === 'delivery' ? 'Ника · 🍕 доставка' : 'Ника · ☕ кофейня');
  if (head) {
...
function dinfoP(){
  ...
  window.__dinfoP = window.__dinfoP ||
    fetch(API_BASE + '/api/delivery/info')
      .then(function(r){ return r.json(); })
      .then(function(x){ window.__dinfo = x; return x; });
...
async function kbAnswer(q, ctx){
  ...
  if (ctx === 'delivery') {
    var d = null;
    try { d = await dinfoP(); } catch(e) {}
    if (/зон|стоимость достав|сколько стоит достав/.test(q) && d)
      return 'Доставка: ' + d.zones.map(function(z){ return z.places.join(', ') + ' — ' + z.fee + ' ₽'; }).join('; ') + '. Самовывоз — бесплатно и −10%.';
    if (/самовывоз/.test(q))
    ...
    if (/акци|подарок|пив|маргарит|бесплатн/.test(q) && d) {
      ...
      if (d.pizzaMonth) s.push('2 пиццы 35 см → «' + d.pizzaMonth.name + '» в подарок');
      return s.length ? 'Сейчас у нас: ' + s.join('; ') : 'Акции обновляются по пятницам — следите за баннером 🍕';
    }
...
window.addMsg = (function(_am){
  return function(who, text){
    ...
    if (who === 'bot' && typeof text === 'string') {
      if (CS.getChatCtx() === 'delivery' && /поддержка «…и кофе»/.test(text)) text = CS.GREET_D;
      if (CS.getChatCtx() === 'coffee' && /поддержка доставки/.test(text)) text = CS.GREET_C;
    }
    var r = _am(who, text);
    if (who === 'system' && !CS.getSupportPending()) setTimeout(showHints, 60);
    return r;
  };
})(window.addMsg);
...
async function mySend(q){
  ...
  if (isHuman) {
    window.addMsg('bot', CS.getChatCtx() === 'delivery' ?
  ...
  var a = null;
  try { a = await kbAnswer(q, CS.getChatCtx() || 'coffee'); } catch(e) {}
  if (!a && typeof _br === 'function') {
    try { a = await _br(q); } catch(e) {}
  }
  var isFallback = !a || /передаю человеку|Приняла!/i.test(a);
  if (isFallback) {
    window.addMsg('bot', 'Хм, не уверена, что поняла 🤔 Хотите, позову сотрудника? Тапните «🙋 Позвать сотрудника» ниже.');
    showHints();
    setTimeout(function(){
...
window.reloadChatThread = async function(){
  ...
  if (!CS.getSupportPending() && !msgs.children.length)
    window.addMsg('bot', CS.getChatCtx() === 'delivery' ? CS.GREET_D : CS.GREET_C);
  showHints();
};
...
window.loadScList = async function(){
  try {
    ...
    document.getElementById('scShowClosed').textContent =
      scClosedView ? '← Активные чаты' : 'Показать закрытые';
    document.getElementById('scList').innerHTML = r.threads.map(function(t){
      return '<div class="hmini" style="cursor:pointer" data-sck="' + esc(t.key) + '">' +
        '<b>' + esc(t.name) + '</b> ' + (t.ctx === 'delivery' ? '🍕' : '☕') +
        (t.human && !scClosedView ? '<span class="tag hit" style="position:static;margin-left:6px">нужен ответ</span>' : '') +
...
document.getElementById('chatMsgs').addEventListener('click',function(e){
  ...
  if(h.dataset.hint===CS.CALL_HINT||h.dataset.arm==='1'){
    if(h.dataset.arm==='1'){
      h.dataset.arm='';h.textContent=CS.CALL_HINT;h.style.background='';h.style.borderColor='';
      mySend(CS.CALL_HINT);
    }else{
      ...
      setTimeout(function(){if(h.dataset.arm==='1'){h.dataset.arm='';h.textContent=CS.CALL_HINT;h.style.background='';h.style.borderColor='';}},4000);
    }
    return;
  }
...
(function(){
  ...
  f.onclick=async function(e){
    ...
    if(CS.getSupportPending()){window.setBotName();window.showSupportOverlay();return;}
    if(CS.getChatCtx()!==brand){CS.setChatCtx(brand);}
    window.setBotName();
